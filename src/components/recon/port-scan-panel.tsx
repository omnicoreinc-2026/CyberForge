...
type SortField = 'port' | 'state' | 'service' | 'version';
      ...
      {status === 'complete' && results && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-6">
          <div className="flex items-center gap-4">
            <div className="glass-card flex items-center gap-2 px-4 py-2"><span className="text-xs text-text-muted">Total:</span><span className="font-mono text-sm font-semibold text-text-primary">{results.total}</span></div>
            <div className="glass-card flex items-center gap-2 px-4 py-2"><span className="text-xs text-text-muted">Open:</span><span className="font-mono text-sm font-semibold text-success">{results.openPorts}</span></div>
          </div>
          <div className="glass-card p-4">
            <h3 className="mb-3 text-xs font-semibold uppercase tracking-wider text-text-muted">Port Map</h3>
            <div className="flex flex-wrap gap-1">{results.ports.map((p) => (<motion.div key={p.port} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: p.port * 0.001 }} title={"Port " + p.port + " - " + p.state + " (" + p.service + ")"} className={cn("h-2.5 w-2.5 rounded-sm", portMapColors[p.state])} />))}</div>
